...
public class BuildingTargetsState implements BuildingState{
    ...
    public Interval refreshTimer = new Interval();
    ...
    public void update(BlockExtended ownerType, BlockExtended.BuildExtended build) {
        if(refreshTimer.get(0, config.refreshTime)) {
            if (lastChange != world.tileChanges) {
                ...
                if (range != null) {
                    targets.clear();
                    indexer.eachBlock(build.team, Tmp.r1.setCentered(build.x, build.y, range.range() * 2), b -> range.inRange(build, b), targets::add);
                }
            }
        }
    }
