...
public class PipelineBridge extends PipelineBlock {
    ...
    public void load() {
        ...
        bridgeBotRegion = Core.atlas.find(name + "-bridge-bot");
    ...
    public class PipelineBridgeBuild extends PipelineBlockBuild {
        ...
        public void doFlow() {
            ...
            if(link != null){
                moveLiquid(link, liquids.current(), idealFlow);
        ...
        public boolean acceptLiquid(Building source, Liquid liquid){
            ...
            if(link == null && !(source instanceof PipelineBridgeBuild b && b.findLink() == this)) return false;
        ...
        public PipelineBridgeBuild[] occupied = new PipelineBridgeBuild[4];
        public @Nullable PipelineBridgeBuild lastLink;
        ...
        @Nullable
        public PipelineBridgeBuild findLink(){
            for(int i = 1; i <= range; i++){
                Tile other = tile.nearby(Geometry.d4x(rotation) * i, Geometry.d4y(rotation) * i);
                if(other != null && other.build instanceof PipelineBridgeBuild build && build.block == PipelineBridge.this && build.team == team){
                    return build;
                }
            }
            return null;
